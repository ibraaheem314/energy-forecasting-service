...
class ForecastRequest(BaseModel):
    ...
    location: str = Field(..., description="Location identifier (e.g., 'region_1', 'grid_A')")
    start_time: datetime = Field(..., description="Start time for forecast period")
    end_time: datetime = Field(..., description="End time for forecast period")
    forecast_type: str = Field(default="consumption", description="Type of forecast: 'consumption' or 'production'")
    resolution: str = Field(default="hourly", description="Forecast resolution: 'hourly', 'daily', 'weekly'")
    confidence_interval: bool = Field(default=True, description="Include confidence intervals in predictions")
...
class ForecastDataPoint(BaseModel):
    ...
    timestamp: datetime
    value: float = Field(..., description="Predicted energy value")
...
class ForecastResponse(BaseModel):
    ...
    predictions: List[ForecastDataPoint] = Field(..., description="Forecast predictions")
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Additional forecast metadata")
